...
if __name__ == "__main__":
    ...
    for snap_number in range(300, 420):
        part = ga.io.Read.read_snapshots(['star', 'dark'],'snapshot', snap_number, simulation_directory, assign_hosts=True, assign_orbits=True, assign_hosts_rotation=False)
        ...
        stars_dist_cut = np.where((stars_dist>30) & (stars_dist<500)) 

        dm_dist = np.sqrt(np.sum(dm_host_distance**2, axis=1))
        dm_dist_cut = np.where((dm_dist>30) & (dm_dist<500)) 
        ...
        dm_figname = "orb_poles_m12b_dark_particles_shell_no_rot_{:03d}.png".format(snap_number)
        mollweide_projection(dm_l_host*180/np.pi, dm_b_host*180/np.pi, [0], [0], 30, 500, bmin=300, bmax=1500, nside=40, figname=dm_figname)
        stars_m12b_kinematics_2 = nba.kinematics.Kinematics(stars_host_distance[stars_dist_cut], stars_host_velocity[stars_dist_cut])
        stars_l_host, stars_b_host = stars_m12b_kinematics_2.pos_cartesian_to_galactic()
        stars_figname = "orb_poles_m12b_stars_particles_shell_no_rot{:03d}.png".format(snap_number)
        mollweide_projection(stars_l_host*180/np.pi, stars_b_host*180/np.pi, [0], [0], 30, 500, bmin=0, bmax=10, nside=40, figname=stars_figname)
