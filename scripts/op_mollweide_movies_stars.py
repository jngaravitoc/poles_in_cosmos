...
if __name__ == "__main__":
    ...
    m12b = FIRE(sim, remove_satellite=True, rm_stellar_sat=True)
    ...
    for k in range(snap_init, snap_final, 1):
        ...
        dist_cut1 = np.where((dist_s> 50) & (dist_s< 300)) 
        dist_cut2 = np.where((dist_s> 300) & (dist_s< 600)) 
        s_kinematics1 = nba.kinematics.Kinematics(pos_s[dist_cut1],  vel_s[dist_cut1])
        s_kinematics2 = nba.kinematics.Kinematics(pos_s[dist_cut2],  vel_s[dist_cut2])
        #m_l_host, dm_b_host = dm_kinematics.pos_cartesian_to_galactic()
        s_OP_l_host1, s_OP_b_host1 = s_kinematics1.orbpole()
        s_OP_l_host2, s_OP_b_host2 = s_kinematics2.orbpole()
        ...
        dm_figname1 =  "../plots/exploration/{}_stelar_OP_satellite_faceon_{:03d}_50_300.png".format(sim, k)
        dm_figname2 =  "../plots/exploration/{}_stelar_OP_satellite_faceon_{:03d}_300_600.png".format(sim, k)

        pl.mollweide_projection(s_OP_l_host1, s_OP_b_host1, sat_OP_l_host[k-300], sat_OP_b_host[k-300], 
                             title=title_dm1, bmin=0, bmax=50,
                             nside=40, smooth=1, figname=dm_figname1)
     
        pl.mollweide_projection(s_OP_l_host2, s_OP_b_host2, sat_OP_l_host[k-300], sat_OP_b_host[k-300], 
                             title=title_dm2, bmin=0, bmax=10,
