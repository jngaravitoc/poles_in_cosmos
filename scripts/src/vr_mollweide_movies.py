...
if __name__ == "__main__":
    ...
    plot_type = 'poles_mollweide'#, orbital_poles 
    ...
    for k in range(snap_init, snap_final, 1):
        ...
        #kin_sat = nba.kinematics.Kinematics(satellite_faceon.dark['pos'], satellite_faceon['vel']*f)
        ...
        if plot_type == "vr_mollweide":        
            ...
            figname = "../plots/exploration/{}_OP_{}_faceon_no_sat_{:03d}.png".format(sim, ptype, k)
            #fig_title = "{}  {}; {}-{} kpc; t={:.2f} Gyr".format(sim, ptype, 50, 300, times[k])
            fig_title = "{}  satellites; {}-{} kpc; t={:.2f} Gyr".format(sim, 0, 400, times[k])
            opl, opb = kinematics1.orbpole()
            #op_lsat, op_bsat = kin_sat.orbpole()
            #pl.multipanel_plot(hfaceon, hfaceon, satellite_faceon, k, sim, figname)
            #pl.mollweide_projection(opl, opb, 
            #                        op_lsat[k-300], 
            #                        op_bsat[k-300], 
            #                        title=fig_title, bmin=bmin, bmax=bmax, nside=40, smooth=4, figname=figname, cmap='inferno')
            
            pl.mollweide_projection(opl, opb, 
                                    opl, opb,
                                    title=fig_title, bmin=bmin, bmax=bmax, nside=40, smooth=4, figname=figname, cmap='Greys')
